...
void main() {
  group('MssqlClient - SQL Injection Security', () {
    ...
    group('easy', () {
      setUpAll(() async {
        await harness.recreateTable(
          'CREATE TABLE dbo.Items (id INT PRIMARY KEY, name NVARCHAR(600) COLLATE Latin1_General_100_CI_AS_SC NOT NULL)',
        );
      });
      ...
      for (var i = 0; i < easyPayloads.length; i++) {
        ...
        test('easy case #$i stores literal safely', () async {
          ...
          final rows = parseRows(
            await harness.query('SELECT name FROM dbo.Items WHERE id=$id'),
          );
    ...
    group('moderate', () {
      setUpAll(() async {
        await harness.recreateTable(
          'CREATE TABLE dbo.Items (id INT PRIMARY KEY, name NVARCHAR(400) NOT NULL)',
        );
      });
      ...
      final moderatePayloads = [
        for (var k = 0; k < 10; k++) ...patterns.map((p) => '$p [$k]'),
      ]; // 100
    ...
    group('hard', () {
      ...
      for (var i = 0; i < whereAttempts.length; i++) {
        ...
        test('hard case #$i WHERE injection yields zero rows', () async {
          final rows = parseRows(
            await harness.executeParams(
              'SELECT id, name FROM dbo.Items WHERE name=@name ORDER BY id',
              {'name': inject},
            ),
          );
    ...
    group('complex', () {
      ...
      final complexPayloads = [
        for (var k = 0; k < 10; k++) ...complexPatterns.map((p) => '$p [$k]'),
      ]; // 100
      ...
      for (var i = 0; i < complexPayloads.length; i++) {
        ...
        test('complex case #$i stacked statements treated as data', () async {
          ...
          expect(t.first['cnt'], 1);
          final rows = parseRows(
            await harness.query('SELECT id, name FROM dbo.Items WHERE id=1'),
          );
